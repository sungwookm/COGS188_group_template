...
# Use the UCI_MOVES mapping from paste-2.txt
# This maps UCI move strings to their indices in the model's output
...
def get_best_move(board, model, device='cpu', temperature=1.0, top_k=5):
    ...
    # Create a mapping from UCI move indices to actual chess.Move objects
    idx_to_uci = {v: k for k, v in UCI_MOVES.items()}
    ...
    if len(valid_moves) > top_k:
        ...
        top_k_valid_indices = [valid_move_indices[i] for i in top_k_indices]
...
def self_play(model, max_moves=100, device='cpu', temperature=1.0):
    """
    ...
    game.headers["Event"] = "Self-play Game"
    ...
    while not board.is_game_over() and move_count < max_moves:
        ...
        move = get_best_move(board, model, device, temperature)
        ...
        # Print the move
        print(f"Move {move_count + 1}: {move.uci()} (by {'White' if board.turn == chess.BLACK else 'Black'})")
...
def main(config: DictConfig) -> None:
    ...
    max_moves = config.self_play.get("max_moves", 100)
    temperature = config.self_play.get("temperature", 1.0)
    
    # Run self-play
    game = self_play(model, max_moves, device, temperature)
    
    # Save the game
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    save_path = f"games/self_play_{timestamp}.pgn"
